...
# 并在离线完成后调用 DeepSeek 生成总结 summary
#
# 输入：WebSocket 二进制 PCM16LE, 16kHz mono
# 输出：
#   - code=0: 实时分段识别结果（data=text，info带时间戳）
#   - code=1: 断开后离线 spk 输出 + deepseek summary（result/summary）
# 同时本地仍会落盘：
...
def _deepseek_summarize_ollama(raw_text: str) -> str:
	...
	prompt = (
			"请根据以下会议内容，优化语句使其更加自然流畅，并生成结构化会议纪要：\n\n"
			+ raw_text
	)
...
async def custom_exception_handler(request: Request, exc: Exception):
	...
	return JSONResponse(
		...
		content=TranscriptionResponse(code=status_code, info=message, data="", summary="", result="").model_dump(),
	)
...
async def ws_transcribe(websocket: WebSocket):
	...
	try:
		...
		try:
			final_obj: Dict[str, Any] = await asyncio.to_thread(offline_postprocess, FULL_WAV_PATH)
		except Exception as e:
		...
		summary_text = "这是测试用的总结占位符"  # 测试用
		
		# 4) 尝试把最终 JSON + summary 回传（客户端若已断开会失败，静默即可）
		try:
			await websocket.send_json(
				TranscriptionResponse(
					code=1,
